...
class pre_conv(nn.Module):
    def __init__(self, in_chans):
        ...
        self.act = nn.GELU()
        self.conv1 = nn.Conv2d(in_chans, in_chans//4, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(in_chans//4)
        self.conv2 = nn.Conv2d(in_chans//4, in_chans, kernel_size=3, padding=1, bias=False)
...
class FAHM(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, num_stages=3,
                 n_groups=[32, 32, 32], embed_dims=[128, 96, 64], depths=[1, 1, 1]):
        super().__init__()
