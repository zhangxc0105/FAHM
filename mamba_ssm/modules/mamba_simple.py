...
class FeatureFusionModule1D(nn.Module):
    def __init__(self, dim):
        super(FeatureFusionModule1D, self).__init__()
        self.conv_fuse = nn.Conv1d(dim, dim, kernel_size=1)
        
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool1d(1),
            nn.Conv1d(dim, dim // 16, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv1d(dim // 16, dim, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, Flow, Fhigh):
        ...
        F_flow_weighted = Flow * W
        F_high_weighted = Fhigh * (1 - W)
        
        F_fused = F_flow_weighted + F_high_weighted
        F_fuse = self.conv_fuse(F_fused)
...
class Mamba(nn.Module):
    def __init__(
        ...
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
        self.fusion = FeatureFusionModule1D(dim = self.d_inner)
    ...
    def forward(self, hidden_states, inference_params=None, T=1):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba:
                ...
                out = self.fusion(out, out_b.flip([-1]))
